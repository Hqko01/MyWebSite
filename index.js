...
leftImages.forEach(element => {
    element.addEventListener('load', () => {
        element.parentElement.parentElement.children[0].classList.remove('loaded')
        setTimeout(() => {
            element.parentElement.parentElement.children[0].classList.add('loaded')
        }, 400);
    })
            ...
            if (element.parentElement.parentElement.children[i] == element.parentElement) {
                if (i == 0) {
                    setTimeout(() => {
                        ...
                        leftImages.forEach(element => {
                            element.attributes['src'].textContent = './HqkoPictures/hqkko.jpg'
                        })
                    }, 400);
                }
                    ...
                    setTimeout(() => {
                        ...
                        leftImages.forEach(element => {
                            element.attributes['src'].textContent = './HqkoPictures/left-background-reference.gif'
                        })
                    }, 400);
                }
            }
